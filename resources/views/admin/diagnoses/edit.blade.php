...
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Diagnosis" />
    ...
    <form method="POST" action="{{ route('admin.diagnoses.update', $diagnosis) }}" class="space-y-4">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $diagnosis->code) }}" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $diagnosis->name) }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">{{ old('description', $diagnosis->description) }}</textarea>
            @error('description')<p class="mt-1 text-theme-xs text-error-500">{{ $message }}</p>@enderror
        </div>
        <div class="flex items-center gap-2">
            <input type="hidden" name="is_active" value="0">
            <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $diagnosis->is_active) ? 'checked' : '' }}>
