...
@section('title', 'Tambah PDF Template')

@section('content')
<x-common.page-breadcrumb pageTitle="Tambah PDF Template" />

<x-common.component-card title="Form PDF Template">
    <form method="POST" action="{{ route('admin.pdf-templates.store') }}" class="space-y-4">
        @csrf
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <select name="type" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">
                    <option value="mcu_letter" {{ old('type') === 'mcu_letter' ? 'selected' : '' }}>MCU Letter</option>
                    <option value="reminder_letter" {{ old('type') === 'reminder_letter' ? 'selected' : '' }}>Reminder Letter</option>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title') }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">
        ...
        <div>
            ...
            <textarea name="combined_html" id="combined_html" rows="16" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-theme-sm dark:border-gray-800 dark:bg-gray-800 dark:text-white/90">{{ old('combined_html') }}</textarea>
