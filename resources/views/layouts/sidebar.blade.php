...
<aside id="sidebar"
    ...
    x-data="{
        ...
        isActive(path) {
            const p = (path || '').startsWith('http') ? new URL(path).pathname : path.replace(/^\//, '');
            const current = window.location.pathname.replace(/^\//, '');
            return current === p || current === ('/' + p).replace(/^\//, '') || '{{ $currentPath }}' === p;
        }
    }"
    :class="{
                        ...
                        <ul class="flex flex-col gap-1">
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                <li>
                                    ...
                                    @else
                                        <!-- Simple Menu Item -->
                                        <a href="{{ $item['path'] }}" target="_self" class="menu-item group"
                                            :class="[
